...
const updateEmployeeManagerPrompt = async () => {
	...
	// This variable will be used to select who the manager is
	const employeesData2 = await getEmployees();
	const managerChoices = employeesData2.map((employee) => {
		return { name: `${employee.first_name} ${employee.last_name}`, value: employee.id };
	});
	// Add 'None' as an option when choosing who the manager is
	...
	return inquirer.prompt([
		{
			message: `Which employee do you want to change who their manager is?`,
			name: 'employeeId',
			type: 'list',
			choices: employeeChoices,
		},
